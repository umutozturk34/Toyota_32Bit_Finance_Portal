...
/**
 * Deposit-holding persistence. Every read is scoped by {@code portfolioId}; callers MUST first verify the
 ...
 */
public interface DepositHoldingRepository extends JpaRepository<DepositHolding, Long> {
    ...
    // making a deleted row appear to "jump". id DESC is the tie-breaker for same-day starts.
    List<DepositHolding> findByPortfolioIdOrderByStartDateDescIdDesc(Long portfolioId);

    List<DepositHolding> findByPortfolioIdAndClosedDateIsNull(Long portfolioId);
