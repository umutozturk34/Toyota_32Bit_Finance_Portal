...
export function inferCouponCadence(history, maturityStart) {
  ...
  for (let i = 1; i < priced.length; i += 1) {
    moves.push({ t: priced[i].t, chg: Math.abs((priced[i].p - priced[i - 1].p) / priced[i - 1].p) * 100 });
  }
  const sortedMoves = moves.map((m) => m.chg).sort((a, b) => a - b);
  const median = sortedMoves[Math.floor(sortedMoves.length / 2)] || 0;
  const threshold = Math.max(1, median * 4);

  // Outlier days, with runs collapsed into a single event (a coupon step can register over a couple of prints).
  const jumps = [];
  for (const m of moves) {
    if (m.chg < threshold) continue;
    if (jumps.length === 0 || (m.t - jumps[jumps.length - 1]) > 10 * DAY_MS) jumps.push(m.t);
    else jumps[jumps.length - 1] = m.t;
  }
  if (jumps.length === 0) return null;

  const startT = maturityStart ? new Date(maturityStart).getTime() : NaN;
  const firstJumpDaysFromStart = Number.isFinite(startT)
    ? Math.round((jumps[0] - startT) / DAY_MS)
    : null;

  if (jumps.length < 2) {
    return { cadenceKey: null, approxDays: null, firstJumpDaysFromStart, jumpCount: jumps.length };
  }

  const gaps = [];
  for (let i = 1; i < jumps.length; i += 1) gaps.push((jumps[i] - jumps[i - 1]) / DAY_MS);
