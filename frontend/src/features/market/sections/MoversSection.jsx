...
function AssetRow({ asset, color, onClick }) {
  ...
  return (
    ...
    >
      ...
      <span className="font-display text-[12px] font-semibold text-fg truncate flex-1 group-hover:text-accent transition-colors">
        {shortLabel(asset)}
      </span>
      <span className="font-mono text-[12px] font-bold text-fg tabular-nums">{formatPriceTRY(asset.price)}</span>
      {asset.changePercent != null && (
        <span className={`font-mono text-[10px] font-semibold tabular-nums min-w-[48px] text-right ${changeColors[cls]}`}>
...
function MoversSectionImpl({ data }) {
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 divide-y sm:divide-y-0 sm:divide-x divide-border-default flex-1 min-h-0">
