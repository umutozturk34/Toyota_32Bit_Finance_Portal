...
export default function PositionsTable({ portfolioId, onAssetClick, onEditClick, onDeleteClick }) {
          ...
          <motion.div key={pos.id} variants={cardVariants} className="rounded-2xl border border-border-default bg-bg-elevated backdrop-blur-md card-hover transition-all duration-200 hover:border-border-hover group">
            <div className="hidden lg:grid lg:grid-cols-[1.3fr_0.7fr_1fr_1fr_1fr_1fr_1.2fr_72px_20px] gap-2 items-center p-4 min-w-0">
              ...
              <div className="flex justify-center gap-1">
                <button onClick={(e) => { e.stopPropagation(); onEditClick(pos); }} className="flex items-center justify-center w-7 h-7 rounded-md text-accent bg-accent/10 hover:bg-accent/20 transition-colors border-none cursor-pointer" aria-label="Düzenle">
                  <Pencil className="h-3 w-3" />
                </button>
                <button onClick={(e) => { e.stopPropagation(); onDeleteClick(pos); }} className="flex items-center justify-center w-7 h-7 rounded-md text-danger bg-danger/10 hover:bg-danger/20 transition-colors border-none cursor-pointer" aria-label="Sil">
