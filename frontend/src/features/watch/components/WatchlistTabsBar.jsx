...
export default function WatchlistTabsBar({ lists, activeId, onSelect, onCreate, onDelete }) {
  ...
  return (
    <div className="flex sm:flex-wrap items-center gap-2 overflow-x-auto sm:overflow-visible -mx-1 px-1 pb-1 sm:pb-0 scroll-smooth">
      {lists.map((list) => {
        ...
        return (
          ...
          >
            ...
            >
              ...
              <span className="truncate max-w-[100px] sm:max-w-[140px]">{watchlistName(t, list)}</span>
      ...
      <button
        ...
        aria-label={t('watch.newListCta')}
        title={t('watch.newListCta')}
        className="inline-flex items-center gap-1 px-2.5 sm:px-3 py-1.5 rounded-lg text-xs font-semibold border border-dashed border-border-default text-fg-muted hover:border-accent hover:text-accent hover:bg-accent/5 transition-colors shrink-0 cursor-pointer bg-transparent"
      >
        <ListPlus className="h-3.5 w-3.5" />
        <span>{t('watch.newListCta')}</span>
      </button>
    </div>
  );
}
