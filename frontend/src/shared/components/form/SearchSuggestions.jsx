...
import RecentSearchRow from './searchSuggestions/RecentSearchRow';
            ...
            <div className="flex-auto min-h-0 overflow-y-auto">
              {recentItems.map((item) => (
                <RecentSearchRow
                  ...
                  onSecondaryAction={(picked) => { secondaryAction.onClick(picked); setQuery(''); setDebouncedQuery(''); setOpen(false); }}
                  onRemove={(picked) => removeRecent({ code: picked.code, type: picked.type })}
                />
              ))}
            </div>
          </motion.div>
