...
<@layout.registrationLayout displayInfo=true displayMessage=!messagesPerField.exists('email_code'); section>
    <#if section = "form">
        ...
        <div class="fp-code-info">
            ...
            <span><strong>${user.email}</strong> adresine gonderilen 6 haneli kodu girin</span>
        </div>

        <form id="kc-verify-email-code-form" action="${url.loginAction}" method="post">
            <div class="form-group">
                <label for="email_code">Dogrulama Kodu</label>
                <input
                    type="text"
                    id="email_code"
                    name="email_code"
    ...
    <#elseif section = "info">
        <div class="fp-resend-box">
            <p>Kodu almadim?
                <a href="${url.loginAction}" class="fp-link">Tekrar Gonder</a>
            </p>
        </div>
    </#if>
